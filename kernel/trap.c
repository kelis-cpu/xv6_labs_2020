    ...
    if (r_scause() == 8) {
        ...
        if (r_scause() == 13 || r_scause() == 15) {
            ...
            for (i = 0; i < MAXVMA; i++) {
                if (p->vmas[i].used &&
                    (faddr >= p->vmas[i].addr &&
                     faddr < p->vmas[i].addr + p->vmas[i].length)) {
                    mem = kalloc();
                    // 读取文件内容
                    memset(mem, 0, PGSIZE);
                    ip = p->vmas[i].mapfile->ip;
                    offset =
                        p->vmas[i].offset +
                        PGROUNDDOWN(faddr - p->vmas[i].addr);  // 计算偏移量

                    ilock(ip);
                    if (readi(ip, 0, (uint64)mem, offset, PGSIZE) <= 0) {
                        p->killed = 1;
                        iunlock(ip);
                        panic("mmap read file fail\n");
                    }
                    iunlock(ip);
                    ...
                    if (mappages(p->pagetable, PGROUNDDOWN(faddr), PGSIZE,
                                 (uint64)mem, perm) != 0) {
                        kfree(mem);
                        p->killed = 1;
                        printf("lazy location fail\n");
                    }
                    break;
                }
            }
            if (i == MAXVMA) p->killed = 1;
        } else {

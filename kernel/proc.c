...
found:
    ...
    // allocate a trapframe for time handler

    if ((p->orignal_trapframe = (struct trapframe *)kalloc()) == 0) {
        release(&p->lock);
        return 0;
    }
...
static void freeproc(struct proc *p) {
    ...
    if (p->orignal_trapframe) kfree((void *)p->orignal_trapframe);
    p->orignal_trapframe = 0;
    p->alarming = 0;
    p->ticks = 0;
    p->alarm_interval = 0;

...
struct thread_ctx {
    uint64 ra;  // return value
    uint64 sp;  // stack pointer

    uint64 s0;  // saved register
    uint64 s1;
    uint64 s2;
    uint64 s3;
    uint64 s4;
...
void thread_schedule(void) {
    ...
    if (current_thread != next_thread) { /* switch threads?  */
         ...
         * thread_switch(??, ??);
         */
        thread_switch((uint64)&t->t_ctx, (uint64)&current_thread->t_ctx);
    } else
        next_thread = 0;
}
...
void thread_create(void (*func)()) {
    ...
    t->state = RUNNABLE;
    // YOUR CODE HERE
    (t->t_ctx).ra = (uint64)func;                     // 设定函数地址
    (t->t_ctx).sp = (uint64)(t->stack + STACK_SIZE);  // 设定栈指针
}

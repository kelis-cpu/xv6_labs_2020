...
void mmap_test(void) {
    ...
    if (p != MAP_FAILED) err("mmap call should have failed");
    if (close(fd) == -1) err("close");
    ...
    if ((fd = open(f, O_RDWR)) == -1) err("open");
    p = mmap(0, PGSIZE * 3, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    //
    int fd1;
    if ((fd1 = open("mmap1", O_RDWR | O_CREATE)) < 0) err("open mmap1");
    if (write(fd1, "12345", 5) != 5) err("write mmap1");
...
void fork_test(void) {
    ...
    if (p1 == MAP_FAILED) err("mmap (4)");
    char *p2 = mmap(0, PGSIZE * 2, PROT_READ, MAP_SHARED, fd, 0);
    if (p2 == MAP_FAILED) err("mmap (5)");
